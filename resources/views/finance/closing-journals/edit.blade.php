    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-lg shadow">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-lg font-medium text-gray-900">Edit Jurnal Penutup</h2>
            </div>
            <form action="{{ route('finance.closing-journals.update', $closingJournal) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="closing_type" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm" required>
                            ...
                            <option value="month_end" {{ old('closing_type', $closingJournal->closing_type) == 'month_end' ? 'selected' : '' }}>Akhir Bulan</option>
                            <option value="year_end" {{ old('closing_type', $closingJournal->closing_type) == 'year_end' ? 'selected' : '' }}>Akhir Tahun</option>
                            <option value="adjustment" {{ old('closing_type', $closingJournal->closing_type) == 'adjustment' ? 'selected' : '' }}>Penyesuaian</option>
                        </select>
                    ...
                    <div>
                        ...
                        <select name="fiscal_period_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm" required>
                            ...
                            @foreach ($fiscalPeriods ?? [] as $period)
                                <option value="{{ $period->id }}" {{ old('fiscal_period_id', $closingJournal->fiscal_period_id) == $period->id ? 'selected' : '' }}>{{ $period->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="description" rows="2" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm" required>{{ old('description', $closingJournal->description) }}</textarea>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 border rounded-md" id="itemsTable">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                @forelse ($items as $idx => $item)
                                    <tr>
                                        <td class="px-4 py-2">
                                            <select name="items[{{ $idx }}][account_id]" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                                                <option value="">Pilih Akun</option>
                                                @foreach ($accounts ?? [] as $account)
                                                    <option value="{{ $account->id }}" {{ (old('items.' . $idx . '.account_id', $item['account_id'] ?? '') == $account->id) ? 'selected' : '' }}>{{ $account->code }} - {{ $account->name }}</option>
                                                @endforeach
                                            </select>
                                        </td>
                                        <td class="px-4 py-2">
                                            <input type="number" step="0.01" min="0" name="items[{{ $idx }}][debit]" value="{{ old('items.' . $idx . '.debit', $item['debit'] ?? 0) }}" class="debit w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                                        </td>
                                        <td class="px-4 py-2">
                                            <input type="number" step="0.01" min="0" name="items[{{ $idx }}][credit]" value="{{ old('items.' . $idx . '.credit', $item['credit'] ?? 0) }}" class="credit w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                                        </td>
                            ...
                            <tfoot class="bg-gray-50">
                                <tr>
                                    ...
                                    <td class="px-4 py-2 text-sm font-medium text-gray-900">
                                        <span id="totalDebit">Rp {{ number_format(collect($items)->sum('debit'), 2, ',', '.') }}</span>
                                    </td>
                                    <td class="px-4 py-2 text-sm font-medium text-gray-900">
                                        <span id="totalCredit">Rp {{ number_format(collect($items)->sum('credit'), 2, ',', '.') }}</span>
                                    </td>
                                    <td></td>
                                </tr>
                            </tfoot>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    tbody.querySelectorAll('.removeRow').forEach(btn => {
    ...
    tbody.querySelectorAll('.debit, .credit').forEach(el => {
        el.addEventListener('input', updateTotals);
    });
});
</script>
