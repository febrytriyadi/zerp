...
<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('purchasing.invoices.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 text-sm">Create New</a>
        </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($purchaseInvoices ?? [] as $invoice)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $invoice->invoice_date }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $invoice->supplier->name ?? $invoice->supplier_id }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600">{{ number_format($invoice->total, 0, ',', '.') }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600">{{ number_format($invoice->outstanding, 0, ',', '.') }}</td>
                            ...
                            <td class="px-6 py-4 text-sm">
                                <a href="{{ route('purchasing.invoices.show', $invoice) }}" class="text-blue-600 hover:text-blue-900 mr-3">View</a>
                                @if ($invoice->status === 'draft')
                                    <a href="{{ route('purchasing.invoices.edit', $invoice) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-10 text-sm text-gray-500 text-center">No purchase invoices found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if (isset($purchaseInvoices) && $purchaseInvoices instanceof \Illuminate\Pagination\LengthAwarePaginator)
