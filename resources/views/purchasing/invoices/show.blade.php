            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white rounded-lg shadow">
                    <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                        <h2 class="text-lg font-medium text-gray-900">Purchase Invoice Detail</h2>
                        <span class="text-sm text-gray-500">#{{ $purchaseInvoice->invoice_number }}</span>
                    </div>
                    <div class="p-6">
                        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <dt class="text-xs font-medium text-gray-500 uppercase">Invoice Number</dt>
                                <dd class="mt-1 text-sm text-gray-900">{{ $purchaseInvoice->invoice_number }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium text-gray-500 uppercase">Invoice Date</dt>
                                <dd class="mt-1 text-sm text-gray-900">{{ $purchaseInvoice->invoice_date }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium text-gray-500 uppercase">Supplier</dt>
                                <dd class="mt-1 text-sm text-gray-900">{{ $purchaseInvoice->supplier->name ?? $purchaseInvoice->supplier_id }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium text-gray-500 uppercase">Due Date</dt>
                                <dd class="mt-1 text-sm text-gray-900">{{ $purchaseInvoice->due_date ?? '-' }}</dd>
                            </div>
                            <div>
                                <dt class="text-xs font-medium text-gray-500 uppercase">Status</dt>
                                <dd class="mt-1">
                                    @if ($purchaseInvoice->status === 'draft') <span class="px-2 py-1 text-xs font-medium bg-yellow-100 text-yellow-700 rounded-full">Draft</span>
                                    @elseif ($purchaseInvoice->status === 'submitted') <span class="px-2 py-1 text-xs font-medium bg-blue-100 text-blue-700 rounded-full">Submitted</span>
                                    @elseif ($purchaseInvoice->status === 'approved') <span class="px-2 py-1 text-xs font-medium bg-green-100 text-green-700 rounded-full">Approved</span>
                                    @elseif ($purchaseInvoice->status === 'rejected') <span class="px-2 py-1 text-xs font-medium bg-red-100 text-red-700 rounded-full">Rejected</span>
                                    @elseif ($purchaseInvoice->status === 'posted') <span class="px-2 py-1 text-xs font-medium bg-purple-100 text-purple-700 rounded-full">Posted</span>
                                    @elseif ($purchaseInvoice->status === 'voided') <span class="px-2 py-1 text-xs font-medium bg-gray-100 text-gray-700 rounded-full">Voided</span>
                                    @else <span class="px-2 py-1 text-xs font-medium bg-gray-100 text-gray-700 rounded-full">{{ $purchaseInvoice->status }}</span>
                                    @endif
                                </dd>
                            </div>
                        </dl>
                        ...
                        <div class="mt-4">
                            ...
                            <dd class="mt-1 text-sm text-gray-900">{{ $purchaseInvoice->description }}</dd>
                        </div>
                        @endif
                        @if ($purchaseInvoice->supplier)
                        <div class="mt-4 p-4 bg-gray-50 rounded-md">
                            <h4 class="text-sm font-medium text-gray-900 mb-2">Supplier Info</h4>
                            <p class="text-sm text-gray-600">{{ $purchaseInvoice->supplier->name }}</p>
                            <p class="text-sm text-gray-600">{{ $purchaseInvoice->supplier->address ?? '-' }}</p>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tfoot>
                                <tr>
                                    ...
                                    <td class="px-3 py-2 text-sm font-medium text-gray-900 text-right">{{ number_format($purchaseInvoice->total, 0, ',', '.') }}</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                </div>
                @if (($purchaseInvoice->supplierPayments ?? collect())->count() > 0)
                    <div class="bg-white rounded-lg shadow">
                        <div class="px-6 py-4 border-b border-gray-200">
                        ...
                        <div class="p-6">
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="divide-y divide-gray-200">
                                    @foreach ($purchaseInvoice->supplierPayments as $payment)
                                        <tr>
                ...
                <div class="bg-white rounded-lg shadow">
                    ...
                    <div class="p-6 space-y-3">
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="font-medium">Rp {{ number_format($purchaseInvoice->total, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex justify-between text-sm">
                            <span class="text-gray-500">Outstanding</span>
                            <span class="font-medium text-red-600">Rp {{ number_format($purchaseInvoice->outstanding_amount, 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>
                <div class="bg-white rounded-lg shadow">
                    ...
                    <div class="p-6 space-y-3">
                        @if ($purchaseInvoice->status === 'draft')
                            <form action="{{ route('purchasing.purchase-invoices.submit', $purchaseInvoice) }}" method="POST">
                                @csrf
                                <button type="submit" class="w-full px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 text-sm">Submit</button>
                            </form>
                        @endif
                        @if ($purchaseInvoice->status === 'submitted')
                            <form action="{{ route('purchasing.purchase-invoices.approve', $purchaseInvoice) }}" method="POST">
                                @csrf
                        ...
                        @if ($purchaseInvoice->status === 'approved')
                            <form action="{{ route('purchasing.purchase-invoices.post', $purchaseInvoice) }}" method="POST">
                                @csrf
                                <button type="submit" class="w-full px-4 py-2 bg-purple-600 text-white rounded-md hover:bg-purple-700 text-sm">Post</button>
                            </form>
                            <form action="{{ route('purchasing.purchase-invoices.void', $purchaseInvoice) }}" method="POST">
                                @csrf
