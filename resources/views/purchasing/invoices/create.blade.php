    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-lg shadow">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-lg font-medium text-gray-900">Create Purchase Invoice</h2>
            </div>
            <form action="{{ route('purchasing.invoices.store') }}" method="POST" class="p-6">
                @csrf
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="invoice_number" value="{{ old('invoice_number', 'PI-' . date('Ymd') . '-001') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm" required>
                    ...
                    <div>
                        ...
                        <input type="date" name="invoice_date" value="{{ old('invoice_date', date('Y-m-d')) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm" required>
                    ...
                    <div>
                        ...
                        <input type="date" name="due_date" value="{{ old('due_date') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm" required>
                    ...
                    <div>
                        ...
                        <select name="purchase_order_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                            ...
                            @foreach ($purchaseOrders ?? [] as $order)
                                <option value="{{ $order->id }}" {{ old('purchase_order_id') == $order->id ? 'selected' : '' }}>{{ $order->order_number }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Received Goods</label>
                        <select name="received_goods_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                            <option value="">None</option>
                            @foreach ($receivedGoods ?? [] as $rg)
                                <option value="{{ $rg->id }}" {{ old('received_goods_id') == $rg->id ? 'selected' : '' }}>{{ $rg->receive_number }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Outstanding Amount <span class="text-red-500">*</span></label>
                        <input type="number" step="0.01" name="outstanding_amount" value="{{ old('outstanding_amount', 0) }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm" required>
                        @error('outstanding_amount') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
