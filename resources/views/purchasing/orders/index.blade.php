...
<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center mb-6">
            ...
            <a href="{{ route('purchasing.orders.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 text-sm">Create New</a>
        </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($purchaseOrders ?? [] as $order)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $order->supplier->name ?? $order->supplier_id }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600">{{ number_format($order->total, 0, ',', '.') }}</td>
                            ...
                            <td class="px-6 py-4 text-sm">
                                <a href="{{ route('purchasing.orders.show', $order) }}" class="text-blue-600 hover:text-blue-900 mr-3">View</a>
                                @if ($order->status === 'draft')
                                    <a href="{{ route('purchasing.orders.edit', $order) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-10 text-sm text-gray-500 text-center">No purchase orders found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if (isset($purchaseOrders) && $purchaseOrders instanceof \Illuminate\Pagination\LengthAwarePaginator)
