    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-lg shadow">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-lg font-medium text-gray-900">Edit Purchase Order</h2>
            </div>
            <form action="{{ route('purchasing.orders.update', $order) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="purchase_request_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                            ...
                            @foreach ($purchaseRequests ?? [] as $req)
                                <option value="{{ $req->id }}" {{ old('purchase_request_id', $order->purchase_request_id) == $req->id ? 'selected' : '' }}>{{ $req->request_number }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
