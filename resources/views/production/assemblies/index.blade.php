...
<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-semibold text-gray-900">Assemblies</h1>
            <a href="{{ route('production.assemblies.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 text-sm">Create New</a>
        </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($assemblies ?? [] as $assembly)
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ $assembly->assembly_number }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $assembly->assembly_date }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $assembly->product->name ?? $assembly->product_id }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $assembly->quantity }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600">{{ number_format($assembly->total_cost, 0, ',', '.') }}</td>
                            ...
                            <td class="px-6 py-4 text-sm">
                                <a href="{{ route('production.assemblies.show', $assembly) }}" class="text-blue-600 hover:text-blue-900 mr-3">View</a>
                                @if ($assembly->status === 'draft')
                                    <a href="{{ route('production.assemblies.edit', $assembly) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-10 text-sm text-gray-500 text-center">No assemblies found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if (isset($assemblies) && $assemblies instanceof \Illuminate\Pagination\LengthAwarePaginator)
