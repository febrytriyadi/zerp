...
<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-6">
            <a href="{{ route('sales.invoices.index') }}" class="text-blue-600 hover:text-blue-900 text-sm">&larr; Back to Sales Invoices</a>
        </div>
        <div class="bg-white rounded-lg shadow">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-lg font-medium text-gray-900">Create Sales Invoice</h2>
            </div>
            <form action="{{ route('sales.invoices.store') }}" method="POST" class="p-6">
                @csrf
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="invoice_number" value="{{ old('invoice_number', 'SI-' . date('Ymd') . '-001') }}" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm" required>
                    ...
                    <div>
                        ...
                        <select name="sales_order_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                            <option value="">None</option>
                            @foreach ($orders ?? [] as $order)
                                <option value="{{ $order->id }}" {{ old('sales_order_id') == $order->id ? 'selected' : '' }}>{{ $order->order_number }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
