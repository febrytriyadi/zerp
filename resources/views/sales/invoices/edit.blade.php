    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white rounded-lg shadow">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-lg font-medium text-gray-900">Edit Sales Invoice</h2>
            </div>
            <form action="{{ route('sales.invoices.update', $invoice) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="sales_order_id" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                            ...
                            @foreach ($orders ?? [] as $order)
                                <option value="{{ $order->id }}" {{ old('sales_order_id', $invoice->sales_order_id) == $order->id ? 'selected' : '' }}>{{ $order->order_number }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
